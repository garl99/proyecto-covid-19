...
create or replace procedure Reporte07(pais varchar2, fecha_i date, fecha_f date, cursore out SYS_REFCURSOR) is
    ...
    begin   
        if((pais is not null) and (fecha_i is not null) and (fecha_f is not null) ) then
            ...
            TO_DATE(fecha_i,'DD/MM/YYYY') and TO_DATE(fecha_f,'DD/MM/YYYY') and
            ...
            TO_DATE(fecha_inicio,'DD/MM/YYYY')= TO_DATE(fecha_f,'DD/MM/YYYY') and
            p.id_pasajero=v.fk_pasajero;  
            ...
            from viaje v, aerolinea a, avion o, persona p
            where v.fk_avion=o.id and o.fk_aerolinea=a.nombre and 
            TO_DATE(fecha_inicio,'DD/MM/YYYY')= TO_DATE(fecha_i,'DD/MM/YYYY') and
            p.id_pasajero=v.fk_pasajero;  
        ...
        else if((pais is not null) and (fecha_i is null) and (fecha_f is null) ) then
            Select id into id_pais from lugar where nombre = pais;
            OPEN cursore FOR
            select 
            a.logo as logo,
            ...
            from viaje v, aerolinea a, avion o, persona p
            where v.fk_avion=o.id and o.fk_aerolinea=a.nombre and p.id_pasajero=v.fk_pasajero
            and id_pais=(select u.fk_lugar from lugar u where u.id=v.fk_destino); 
        
        else if((pais is null) and (fecha_i is null) and (fecha_f is null) ) then
            Select id into id_pais from lugar where nombre = pais;
            OPEN cursore FOR
            select 
            a.logo as logo,
            ...
            from viaje v, aerolinea a, avion o, persona p
            where v.fk_avion=o.id and p.id_pasajero=v.fk_pasajero and o.fk_aerolinea=a.nombre; 
...
set autoprint on;
variable cursore refcursor;
execute Reporte07('Estados Unidos',null, null,:cursore);
